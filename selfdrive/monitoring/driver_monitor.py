...
class DRIVER_MONITOR_SETTINGS:
    def __init__(self, TICI=TICI, DT_DMON=DT_DMON):
        ...
        self._AWARENESS_TIME = 35.0  # passive wheeltouch total timeout
        self._AWARENESS_PRE_TIME_TILL_TERMINAL = 12.0
        self._AWARENESS_PROMPT_TIME_TILL_TERMINAL = 6.0
        self._DISTRACTED_TIME = 11.0  # active monitoring total timeout
        self._DISTRACTED_PRE_TIME_TILL_TERMINAL = 8.0
        self._DISTRACTED_PROMPT_TIME_TILL_TERMINAL = 6.0
        ...
        self._HI_STD_FALLBACK_TIME = int(
            10 / self._DT_DMON
        )  # fall back to wheel touch if model is uncertain for 10s
        self._DISTRACTED_FILTER_TS = 0.25  # 0.6Hz

        self._POSE_CALIB_MIN_SPEED = 13  # 30 mph
        self._POSE_OFFSET_MIN_COUNT = int(
            60 / self._DT_DMON
        )  # valid data counts before calibration completes, 1min cumulative
        self._POSE_OFFSET_MAX_COUNT = int(
            360 / self._DT_DMON
        )  # stop deweighting new data after 6 min, aka "short term memory"

        self._RECOVERY_FACTOR_MAX = 5.0  # relative to minus step change
        self._RECOVERY_FACTOR_MIN = 1.25  # relative to minus step change

        self._MAX_TERMINAL_ALERTS = 3  # not allowed to engage after 3 terminal alerts
        self._MAX_TERMINAL_DURATION = int(
            30 / self._DT_DMON
        )  # not allowed to engage after 30s of terminal alerts
...
class DriverStatus:
    def __init__(self, rhd=False, settings=DRIVER_MONITOR_SETTINGS()):
        ...
        self.driver_distraction_filter = FirstOrderFilter(
            0.0, self.settings._DISTRACTED_FILTER_TS, self.settings._DT_DMON
        )
        ...
        self.hi_stds = 0
        self.threshold_pre = (
            self.settings._DISTRACTED_PRE_TIME_TILL_TERMINAL
            / self.settings._DISTRACTED_TIME
        )
        self.threshold_prompt = (
            self.settings._DISTRACTED_PROMPT_TIME_TILL_TERMINAL
            / self.settings._DISTRACTED_TIME
        )
    ...
    def _set_timers(self, active_monitoring):
        if self.active_monitoring_mode and self.awareness <= self.threshold_prompt:
            if active_monitoring:
                self.step_change = (
                    self.settings._DT_DMON / self.settings._DISTRACTED_TIME
                )
            else:
                self.step_change = 0.0
            return  # no exploit after orange alert
        elif self.awareness <= 0.0:
            return
        ...
        if active_monitoring:
            ...
            self.threshold_pre = (
                self.settings._AWARENESS_PRE_TIME_TILL_TERMINAL
                / self.settings._AWARENESS_TIME
            )
            self.threshold_prompt = (
                self.settings._AWARENESS_PROMPT_TIME_TILL_TERMINAL
                / self.settings._AWARENESS_TIME
            )
    ...
    def _is_driver_distracted(self, pose, blink):
        ...
        pitch_error = (
            0 if pitch_error > 0 else abs(pitch_error)
        )  # no positive pitch limit
        yaw_error = abs(yaw_error)

        if (
            pitch_error * self.settings._PITCH_WEIGHT
            > self.settings._METRIC_THRESHOLD * pose.cfactor
            or yaw_error > self.settings._METRIC_THRESHOLD * pose.cfactor
        ):
            return DistractedType.BAD_POSE
        elif (
            blink.left_blink + blink.right_blink
        ...
        self.pose.cfactor = (
            interp(
                ...
                [
                    ...
                    self.settings._METRIC_THRESHOLD_SLACK,
                ],
            )
            / self.settings._METRIC_THRESHOLD
        )
    ...
    def get_pose(self, driver_state, cal_rpy, car_speed, op_engaged):
        ...
        self.face_partial = (
            driver_state.partialFace > self.settings._PARTIAL_FACE_THRESHOLD
        )
        self.face_detected = (
            driver_state.faceProb > self.settings._FACE_THRESHOLD or self.face_partial
        )
        ...
        self.pose.low_std = (
            model_std_max < self.settings._POSESTD_THRESHOLD and not self.face_partial
        )
        self.blink.left_blink = (
            driver_state.leftBlinkProb
            * (driver_state.leftEyeProb > self.settings._EYE_THRESHOLD)
            * (driver_state.sunglassesProb < self.settings._SG_THRESHOLD)
        )
        self.blink.right_blink = (
            driver_state.rightBlinkProb
            * (driver_state.rightEyeProb > self.settings._EYE_THRESHOLD)
            * (driver_state.sunglassesProb < self.settings._SG_THRESHOLD)
        )

        self.driver_distracted = (
            self._is_driver_distracted(self.pose, self.blink) > 0
            and driver_state.faceProb > self.settings._FACE_THRESHOLD
        ...
        if (
            self.face_detected
            and car_speed > self.settings._POSE_CALIB_MIN_SPEED
            and self.pose.low_std
            and (not op_engaged or not self.driver_distracted)
        ):
            self.pose.pitch_offseter.push_and_update(self.pose.pitch)
            self.pose.yaw_offseter.push_and_update(self.pose.yaw)

        self.pose_calibrated = (
            self.pose.pitch_offseter.filtered_stat.n
            > self.settings._POSE_OFFSET_MIN_COUNT
            and self.pose.yaw_offseter.filtered_stat.n
            > self.settings._POSE_OFFSET_MIN_COUNT
        )
    ...
    def update(self, events, driver_engaged, ctrl_active, standstill):
        # Driver monitoring disabled by user request
        self.awareness = 1.0
        self.awareness_active = 1.0
        self.awareness_passive = 1.0
        self.terminal_alert_cnt = 0
