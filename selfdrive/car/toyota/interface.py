...
class CarInterface(CarInterfaceBase):
    ...
    def get_params(candidate, fingerprint=gen_empty_fingerprint(), car_fw=[]):  # pylint: disable=dangerous-default-value
        ...
        ret.stoppingControl = (
            False  # Toyota starts braking more when it thinks you want to stop
        )
        ...
        if candidate == CAR.PRIUS:
            ret.safetyConfigs[
                0
            ].safetyParam = 66  # see conversion factor for STEER_TORQUE_EPS in dbc file
            stop_and_go = True
            ret.wheelbase = 2.70
            ret.steerRatio = 15.74  # unknown end-to-end spec
            tire_stiffness_factor = 0.6371  # hand-tune
            ret.mass = 3045.0 * CV.LB_TO_KG + STD_CARGO_KG
            ...
            ret.mass = (
                3650.0 * CV.LB_TO_KG + STD_CARGO_KG
            )  # mean between normal and hybrid
            ...
            ret.mass = (
                2860.0 * CV.LB_TO_KG + STD_CARGO_KG
            )  # mean between normal and hybrid
            ...
            ret.steerRatio = 14.8
            tire_stiffness_factor = 0.5533
            ret.mass = 4387.0 * CV.LB_TO_KG + STD_CARGO_KG
            ...
            ret.mass = 3300.0 * CV.LB_TO_KG + STD_CARGO_KG
            ...
            tire_stiffness_factor = 0.7933
            ret.mass = (
                3400.0 * CV.LB_TO_KG + STD_CARGO_KG
            )  # mean between normal and hybrid
            set_lat_tune(ret.lateralTuning, LatTunes.PID_C)
            ...
            ret.mass = 4700.0 * CV.LB_TO_KG + STD_CARGO_KG  # 4260 + 4-5 people
            ...
            tire_stiffness_factor = 0.8
            ret.mass = (
                4607.0 * CV.LB_TO_KG + STD_CARGO_KG
            )  # mean between normal and hybrid limited
            set_lat_tune(ret.lateralTuning, LatTunes.PID_G)
            ...
            ret.mass = (
                3505.0 * CV.LB_TO_KG + STD_CARGO_KG
            )  # mean between normal and hybrid
            ...
            tire_stiffness_factor = 0.7933
            ret.mass = (
                3585.0 * CV.LB_TO_KG + STD_CARGO_KG
            )  # Average between ICE and Hybrid
            set_lat_tune(ret.lateralTuning, LatTunes.PID_D)
            ...
            for fw in car_fw:
                if fw.ecu == "eps" and (
                    fw.fwVersion.startswith(b"\x02")
                    or fw.fwVersion in [b"8965B42181\x00\x00\x00\x00\x00\x00"]
                ):
            ...
            stop_and_go = True
            ret.wheelbase = (
                2.67  # Average between 2.70 for sedan and 2.64 for hatchback
            )
            ret.steerRatio = 13.9
            tire_stiffness_factor = 0.444  # not optimized yet
            ret.mass = 3060.0 * CV.LB_TO_KG + STD_CARGO_KG
            set_lat_tune(ret.lateralTuning, LatTunes.PID_D)
            ...
            tire_stiffness_factor = 0.444  # not optimized yet
            ret.mass = 3704.0 * CV.LB_TO_KG + STD_CARGO_KG
            set_lat_tune(ret.lateralTuning, LatTunes.PID_D)
            ...
            tire_stiffness_factor = 0.444  # not optimized yet
            ret.mass = 3682.0 * CV.LB_TO_KG + STD_CARGO_KG
            set_lat_tune(ret.lateralTuning, LatTunes.PID_D)
            ...
            ret.mass = 4590.0 * CV.LB_TO_KG + STD_CARGO_KG
            ...
            tire_stiffness_factor = 0.6371  # hand-tune
            ret.mass = 3115.0 * CV.LB_TO_KG + STD_CARGO_KG
            ...
            ret.steerRatio = 14.8
            tire_stiffness_factor = 0.8
            ret.mass = 4300.0 * CV.LB_TO_KG + STD_CARGO_KG
            set_lat_tune(ret.lateralTuning, LatTunes.PID_C)
            ...
            ret.steerRatio = 14.2
            tire_stiffness_factor = 0.444
            ret.mass = 4305.0 * CV.LB_TO_KG + STD_CARGO_KG
            set_lat_tune(ret.lateralTuning, LatTunes.PID_J)
        ...
        ret.enableDsu = (
            (len(found_ecus) > 0)
            and (Ecu.dsu not in found_ecus)
            and (candidate not in NO_DSU_CAR)
            and (not smartDsu)
        )
        ...
        ret.minEnableSpeed = (
            -1.0 if (stop_and_go or ret.enableGasInterceptor) else MIN_ACC_SPEED
        )
    ...
    def update(self, c, can_strings):
        ...
        ret.steeringRateLimited = (
            self.CC.steer_rate_limited if self.CC is not None else False
        )
        ...
        if self.CS.cruise_buttons != self.CS.prev_cruise_buttons:
            ...
            if self.CS.cruise_buttons in [
                CruiseButtons.ACCEL_ACC,
                CruiseButtons.ACCEL_CC,
                CruiseButtons.DECEL_ACC,
                CruiseButtons.DECEL_CC,
            ]:
                be.pressed = True
        ...
        extraGears = []
        if not (
            self.CS.CP.openpilotLongitudinalControl or self.CS.CP.enableGasInterceptor
        ):
            extraGears = [
        ...
        events = self.create_common_events(
            ret, extra_gears=extraGears, pcm_enable=False
        )
        ...
        if (
            self.CS.disengageByBrake
            and not ret.brakePressed
            and not ret.brakeHoldActive
            and self.CS.lkasEnabled
        ):
            enable_pressed = True
