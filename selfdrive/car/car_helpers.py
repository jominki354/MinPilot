...
def load_interfaces(brand_names):
    ...
    for brand_name in brand_names:
        ...
        CarInterface = __import__(
            path + ".interface", fromlist=["CarInterface"]
        ).CarInterface
        ...
        if os.path.exists(BASEDIR + "/" + path.replace(".", "/") + "/carcontroller.py"):
            CarController = __import__(
                path + ".carcontroller", fromlist=["CarController"]
            ).CarController
        else:
            CarController = None
...
def _get_interface_names():
    ...
    for car_folder in [x[0] for x in os.walk(BASEDIR + "/selfdrive/car")]:
        try:
            ...
            model_names = __import__(
                "selfdrive.car.%s.values" % brand_name, fromlist=["CAR"]
            ).CAR
            model_names = [
                getattr(model_names, c)
                for c in model_names.__dict__.keys()
...
def fingerprint(logcan, sendcan):
    ...
    if not fixed_fingerprint and not skip_fw_query:
        ...
        cached_params = SafeParams().get("CarParamsCache")
        ...
        if (
            ...
            and len(cached_params.carFw) > 0
            and cached_params.carVin is not VIN_UNKNOWN
        ):
    ...
    candidate_cars = {
        i: all_legacy_fingerprint_cars() for i in [0, 1]
    ...
    while not done:
        ...
        for can in a.can:
            ...
            for b in candidate_cars:
                ...
                if (
                    can.src == b
                    and can.address < 0x800
                    and can.address not in [0x7DF, 0x7E0, 0x7E8]
                ):
        ...
        failed = (
            all(len(cc) == 0 for cc in candidate_cars.values())
            and frame > frame_fingerprint
        ) or frame > 200
...
def crash_log2(fingerprints, fw):
    while True:
        if is_connected_to_internet():
            crash.capture_warning(
                "car doesn't match any fingerprints: %s" % fingerprints
            )
...
def get_car(logcan, sendcan):
    ...
    if SafeParams().get("CarModel", encoding="utf8") is not None:
        car_name = SafeParams().get("CarModel", encoding="utf8")
    ...
    if candidate is None:
        ...
        # Relaxed condition: core IDs + at least 3 of 5 total IDs
        toyota_core_ids = {0x1D2, 0x2E4}  # PCM_CRUISE, STEERING_LKA (essential)
        toyota_tss2_ids = {
            0x1D2,  # PCM_CRUISE
            0x1D3,  # PCM_CRUISE_2
            0x191,  # STEERING_LTA
        ...
        matched_ids = toyota_tss2_ids & can_ids

        if toyota_core_ids.issubset(can_ids) and len(matched_ids) >= 3:
            # Toyota TSS2 pattern detected - default to CAMRYH_TSS2
            from selfdrive.car.toyota.values import CAR as TOYOTA

            candidate = TOYOTA.CAMRYH_TSS2
            cloudlog.warning(
                "FW fingerprint failed, using CAN fallback: %s (matched %d/5 IDs)",
                candidate,
                len(matched_ids),
            )
        else:
