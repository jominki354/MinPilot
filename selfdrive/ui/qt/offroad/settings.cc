...
SunnypilotPanel::SunnypilotPanel(QWidget* parent) : QWidget(parent) {
  ...
  // SAFETY: 판다 연결 시 ui_dev_publisher가 자동 종료됨
  auto uiDevModeToggle = new ParamControl("UIDevMode", "UI 개발 모드", "판다/차량 연결 없이 온로드 UI를 테스트합니다.", "../assets/offroad/icon_calibration.png", this);
  QObject::connect(uiDevModeToggle, &ParamControl::toggleFlipped, [=](bool state) {
    if (state) {
      // Start ui_dev_publisher
