...
void Networking::connectToNetwork(const Network &n) {
  if (wifi->isKnownConnection(n.ssid)) {
    ...
    QString pass = InputDialog::getText("비밀번호 입력", this, "\"" + n.ssid + "\" 비밀번호 입력", true, 8);
    if (!pass.isEmpty()) {
      wifi->connect(n, pass);
    }
  }
...
void Networking::wrongPassword(const QString &ssid) {
  if (wifi->seenNetworks.contains(ssid)) {
    ...
    QString pass = InputDialog::getText("비밀번호 오류", this, "\"" + n.ssid +"\" 비밀번호를 다시 입력하세요", true, 8);
    if (!pass.isEmpty()) {
      wifi->connect(n, pass);
    }
  }
...
AdvancedNetworking::AdvancedNetworking(QWidget* parent, WifiManager* wifi): QWidget(parent), wifi(wifi) {
  ...
  ButtonControl *editPasswordButton = new ButtonControl("테더링 비밀번호", "변경");
  connect(editPasswordButton, &ButtonControl::clicked, [=]() {
    QString pass = InputDialog::getText("새 테더링 비밀번호 입력", this, "", true, 8, wifi->getTetheringPassword());
    if (!pass.isEmpty()) {
  ...
  // APN settings
  ButtonControl *editApnButton = new ButtonControl("APN 설정", "변경");
  connect(editApnButton, &ButtonControl::clicked, [=]() {
    const bool roamingEnabled = params.getBool("GsmRoaming");
    const QString cur_apn = QString::fromStdString(params.get("GsmApn"));
    QString apn = InputDialog::getText("APN 입력", this, "자동 설정을 위해 비워두세요", false, -1, cur_apn).trimmed();
...
void WifiUI::refresh() {
  ...
  for (Network &network : sortedNetworks) {
    ...
    if (network.connected == ConnectedType::CONNECTING) {
      QPushButton *connecting = new QPushButton("연결 중...");
    ...
    if (wifi->isKnownConnection(network.ssid) && !wifi->isTetheringEnabled()) {
      QPushButton *forgetBtn = new QPushButton("삭제");
      forgetBtn->setObjectName("forgetBtn");
      QObject::connect(forgetBtn, &QPushButton::clicked, [=]() {
        if (ConfirmationDialog::confirm("Wi-Fi 네트워크 \"" + QString::fromUtf8(network.ssid) + "\"를 삭제하시겠습니까?", this)) {
