...
def main():
    ...
    print(f"[carrotd] Started on port {PORT}")
    last_recv = 0
    ...
    while True:
        try:
            ...
            sdi_block_dist = int(d.get("nSdiBlockDist", 0))
            ...
            if sdi_type in FIXED_CAMERA and sdi_limit > 0 and sdi_dist > 0:
                target_speed = calc_speed(sdi_dist, sdi_limit, SAFE_TIME, DECEL_RATE)
            ...
            # 4. 과속방지턱 (옵션)
            elif (sdi_type in SPEED_BUMP or sdi_plus_type in SPEED_BUMP) and use_bump:
