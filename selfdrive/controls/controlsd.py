...
class Controls:
    def __init__(self, sm=None, pm=None, can_sock=None):
        ...
        controller_available = (
            self.CI.CC is not None and not passive and not self.CP.dashcamOnly
        )
        community_feature = (
            self.CP.communityFeature
            or self.CP.fingerprintSource == car.CarParams.FingerprintSource.can
        )
        ...
        self.read_only = (
            not car_recognized
            or not controller_available
            or self.CP.dashcamOnly
            or community_feature_disallowed
        )
        if self.read_only:
        ...
        self.carrot_params = Params("/dev/shm/params")
        ...
        self.startup_event = get_startup_event(
            car_recognized, controller_available, len(self.CP.carFw) > 0
        )
    ...
    def update_events(self, CS):
        ...
        if (
            EON
            and (self.sm["peripheralState"].pandaType != PandaType.uno)
            and self.sm["deviceState"].batteryPercent < 1
            and self.sm["deviceState"].chargingError
        ):
        ...
        if (
            self.sm["deviceState"].memoryUsagePercent > (90 if TICI else 65)
            and not SIMULATION
        ):
        ...
        if self.sm["peripheralState"].pandaType in [PandaType.uno, PandaType.dos]:
            if (
                self.sm["peripheralState"].fanSpeedRpm == 0
                and self.sm["deviceState"].fanSpeedPercentDesired > 50
            ):
        ...
        # Handle lane change
        lane_change_set_timer = int(
            Params().get("AutoLaneChangeTimer", encoding="utf8")
        )
        ...
        if not self.sm["liveLocationKalman"].sensorsOK and not NOSENSOR:
            if (
                self.sm.frame > 5 / DT_CTRL
            ):  # Give locationd some time to receive all the inputs
        ...
        if not REPLAY:
            ...
            self.cruise_mismatch_counter = (
                self.cruise_mismatch_counter + 1 if cruise_mismatch else 0
            )
            if self.cruise_mismatch_counter > int(3.0 / DT_CTRL):
                self.events.add(EventName.cruiseMismatch)

        # Check for FCW
        stock_long_is_braking = (
            self.enabled and not self.CP.openpilotLongitudinalControl and CS.aEgo < -1.5
        )
        model_fcw = (
        ...
        if (
            CS.brakePressed
            and v_future >= self.CP.vEgoStarting
    ...
    def data_sample(self):
        ...
        if not self.initialized and (
            all_valid or self.sm.frame * DT_CTRL > 3.5 or SIMULATION
        ):
        ...
        if any(
            not ps.controlsAllowed and self.enabled
    ...
    def state_transition(self, CS):
            ...
            if self.events.any(ET.USER_DISABLE):
                ...
                # SOFT DISABLING
                    ...
                    if not self.events.any(ET.SOFT_DISABLE):
                        ...
                        self.events.any(ET.SOFT_DISABLE) and self.soft_disable_timer > 0
                    ):
                        self.current_alert_types.append(ET.SOFT_DISABLE)
        ...
        # CarrotMan 감속 적용
        try:
            if self.params.get_bool("CarrotSpeedControl"):
                carrot_limit = int(self.carrot_params.get("CarrotSpdLimit") or 0)
                if carrot_limit > 0 and self.v_cruise_kph > carrot_limit:
                    self.v_cruise_kph = carrot_limit
        except:
            pass
    ...
    def state_control(self, CS):
        ...
        if not self.joystick_mode:
            ...
            actuators.accel = self.LoC.update(
                long_active, CS, self.CP, long_plan, pid_accel_limits
            )

            # Steering PID loop and lateral MPC
            lat_active = (
                self.active
                and not CS.steerWarning
                and not CS.steerError
                and CS.vEgo > self.CP.minSteerSpeed
                and (CS.lfaEnabled or CS.accMainEnabled or CS.lkasEnabled)
                and (not CS.belowLaneChangeSpeed)
                or (not (((self.sm.frame - self.last_blinker_frame) * DT_CTRL) < 1.0))
            )
            ...
            if self.sm.rcv_frame["testJoystick"] > 0 and self.active:
                ...
                actuators.steer, actuators.steeringAngleDeg = steer, steer * 45.0
        ...
        angle_control_saturated = (
            ...
            and abs(actuators.steeringAngleDeg - CS.steeringAngleDeg)
    ...
    def publish_logs(self, CS, start_time, actuators, lac_log):
        ...
        if (
            self.joystick_mode
            and self.sm.rcv_frame["testJoystick"] > 0
            and self.sm["testJoystick"].buttons[0]
        ):
        ...
        force_decel = (self.sm["driverMonitoringState"].awarenessStatus < 0.0) or (
            self.state == State.softDisabling
        )
        ...
        curvature = -self.VM.calc_curvature(
            steer_angle_without_offset, CS.vEgo, params.roll
        )
        ...
        controlsState.cumLagMs = -self.rk.remaining * 1000.0
        ...
        controlsState.suspended = (CS.brakePressed and CS.disengageByBrake) or (
            ...
            and not (
                ...
                and (CS.lfaEnabled or CS.accMainEnabled or CS.lkasEnabled)
                and (
                    (not CS.belowLaneChangeSpeed)
                    or (
                        not (
                            ((self.sm.frame - self.last_blinker_frame) * DT_CTRL) < 1.0
                        )
                    )
                )
            )
        ...
        if (self.sm.frame % int(1.0 / DT_CTRL) == 0) or (
            self.events.names != self.events_prev
        ):
