...
class LateralPlanner:
    def __init__(self, CP, use_lanelines=True, wide_camera=False):
        ...
        self.dynamic_lane_profile = int(
            self.params.get("DynamicLaneProfile", encoding="utf8")
        )
    ...
    def update(self, sm):
        ...
        if self.second > 1.0:
            ...
            self.dynamic_lane_profile = int(
                Params().get("DynamicLaneProfile", encoding="utf8")
            )
            self.second = 0.0
        ...
        lane_change_auto_timer = (
            ...
            if lane_change_set_timer == 1
            else 0.5
            if lane_change_set_timer == 2
            else 1.0
            if lane_change_set_timer == 3
            else 1.5
            if lane_change_set_timer == 4
            else 2.0
        )
        ...
        if (not active) or (self.lane_change_timer > LANE_CHANGE_TIME_MAX):
            ...
            # LaneChangeState.laneChangeStarting
                ...
                # 98% certainty
                lane_change_prob = (
                    self.LP.l_lane_change_prob + self.LP.r_lane_change_prob
                )
        ...
        if (
            self.desire == log.LateralPlan.Desire.laneChangeRight
            or self.desire == log.LateralPlan.Desire.laneChangeLeft
        ):
        ...
        if self.get_dynamic_lane_profile():
            ...
            path_cost = (
                np.clip(abs(self.path_xyz[0, 1] / self.path_xyz_stds[0, 1]), 0.5, 1.5)
                * MPC_COST_LAT.PATH
            )
            ...
            self.lat_mpc.set_weights(
                MPC_COST_LAT.PATH, MPC_COST_LAT.HEADING, self.steer_rate_cost
            )
            self.dynamic_lane_profile_status = False

        # PathOffset 업데이트 (1초 주기)
        if self.second == 0.0:
            if self.params.get_bool("PathOffsetEnable"):
                self.path_offset = (
                    float(self.params.get("PathOffset", encoding="utf8") or 0) * 0.01
                )
            else:
        ...
        self.x0[4] = v_ego
        self.lat_mpc.run(self.x0, v_ego, CAR_ROTATION_RADIUS, y_pts, heading_pts)
        # init state for next
        self.x0[3] = interp(
            DT_MDL, self.t_idxs[: LAT_MPC_N + 1], self.lat_mpc.x_sol[:, 3]
        )
        ...
        if self.lat_mpc.cost > 20000.0 or mpc_nans:
    ...
    def get_dynamic_lane_profile(self):
        ...
        if self.dynamic_lane_profile == 0:
            ...
            if self.lane_change_state == LaneChangeState.off:
                ...
                ):  # in buffer mode, always laneless
                    return True
        return False
    ...
    def publish(self, sm, pm):
        ...
        plan_send.lateralPlan.dynamicLaneProfile = bool(
