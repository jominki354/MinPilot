...
def get_param_float(params, key, default=0.0):
    """C2 호환 파라미터 읽기"""
    try:
        val = params.get(key)
        if val is None:
...
class LatControlTorque(LatControl):
    def __init__(self, CP, CI):
        ...
        self.params = Params()
        self.lateralTorqueCustom = get_param_int(self.params, "LateralTorqueCustom", 0)
        self.lateralTorqueAccelFactor = (
            get_param_float(self.params, "LateralTorqueAccelFactor", 2500) * 0.001
        )
        self.lateralTorqueFriction = (
            get_param_float(self.params, "LateralTorqueFriction", 100) * 0.001
        )
    ...
    def update_params(self):
        ...
        if self.paramsCount > 30:
            ...
            self.lateralTorqueCustom = get_param_int(
                self.params, "LateralTorqueCustom", 0
            )
            self.lateralTorqueAccelFactor = (
                get_param_float(self.params, "LateralTorqueAccelFactor", 2500) * 0.001
            )
        ...
        if not active:
            ...
            if self.use_steering_angle:
                ...
                actual_curvature = interp(
                    CS.vEgo, [2.0, 5.0], [actual_curvature_vm, actual_curvature_llk]
                )
            ...
            gravity_adjusted_lateral_accel = (
                desired_lateral_accel - params.roll * ACCELERATION_DUE_TO_GRAVITY
            )
            ...
            self.latDebugText = "latAccel={:1.3f},Friction={:1.3f}".format(
