...
LIMIT_MIN_ACC = (
    -1.5
)  # m/s^2 Maximum deceleration allowed for limit controllers to provide.
LIMIT_MAX_ACC = 1.0  # m/s^2 Maximum acelration allowed for limit controllers to provide while active.
LIMIT_MIN_SPEED = (
    8.33  # m/s, Minimum speed limit to provide as solution on limit controllers.
)
LIMIT_SPEED_OFFSET_TH = (
    -1.0
)  # m/s Maximum offset between speed limit and current speed for adapting state.
...
def update_v_cruise(v_cruise_kph, buttonEvents, button_timers, enabled, metric):
    ...
    if button_type:
        if Params().get_bool("CustomAccIncrementsEnabled"):
            short_increment = float(
                Params().get("CustomAccIncrementsShort", encoding="utf8")
            )
            long_increment = float(
                Params().get("CustomAccIncrementsLong", encoding="utf8")
            )
            v_cruise_delta = long_increment if long_press else short_increment
        else:
            v_cruise_delta = v_cruise_delta * (5 if long_press else 1)

        if long_press and v_cruise_kph % v_cruise_delta != 0:  # partial interval
            v_cruise_kph = (
                CRUISE_NEAREST_FUNC[button_type](v_cruise_kph / v_cruise_delta)
                * v_cruise_delta
            )
        else:
...
def get_lag_adjusted_curvature(CP, v_ego, psis, curvatures, curvature_rates):
    ...
    safe_desired_curvature_rate = clip(
        desired_curvature_rate, -max_curvature_rate, max_curvature_rate
    )
